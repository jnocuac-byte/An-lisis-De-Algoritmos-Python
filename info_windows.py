...
class ProgramInfoWindow:
   ...
   def __init__(self, parent):
      ...
      self.bg_color = self.colors['bg_primary']
   ...
   def setup_ui(self):
      ...
      style.configure(
         'CustomFrame.TFrame', 
         background=self.bg_color # Usa el color que quieres para el fondo
      )

      # Frame principal con scroll
      main_frame = ttk.Frame(self.window, style='CustomFrame.TFrame')
      main_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
      
      # Canvas para scroll
      canvas = tk.Canvas(main_frame, highlightthickness=0, bg=self.bg_color)
      scrollbar = ttk.Scrollbar(main_frame, orient="vertical", command=canvas.yview)
      scrollable_frame = ttk.Frame(canvas, style='CustomFrame.TFrame')
